...
public class Matrix {
	...
	public Matrix(int row, int col, String values) {
		...
		boolean withinWeight = false;
		boolean withinRow = false;
		String curValue = "";
		for (int y = 0; y < row;) {
			for (int x = 0; x < col;) {
				char curChar = values.charAt(characterIndex);
				if ((curChar == '[') && !withinWeight) {
					// entering a weight matrix
					withinWeight = true;
				} else if ((curChar == '[') && withinWeight) {
					// within weight matrix
					withinRow = true;
				} else if ((curChar == ']') && withinRow) {
					// within weight matrix and ended curent value
					try {
	...
	public static Matrix MatrixProduct(Matrix m1, Matrix m2) {
		...
		if (m1.col == m2.row) {
			...
			System.out.println("You just tried to matrix product a matrix that doesn't have equal row to the others cols! ");
		}
